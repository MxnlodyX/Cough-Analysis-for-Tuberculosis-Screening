"""
Implementation of Fusion Feature (MFCC + Mel-Spectrogram + Spectral Contrast) based HistGradientBoosting model
...
class ExperimentConfig:
	...
	# HistGradientBoosting from notebook
	...
	output_rel_dir: str = "output_hgboost_audio"
	cache_rel_dir: str = "cache_hgboost_features"
...
def parse_cli_args() -> argparse.Namespace:
	parser = argparse.ArgumentParser(
		description=(
			"HistGradientBoosting on CODA wavs with Capsule-style split "
	...
	parser.add_argument("--max-depth", type=int, default=None, help="Override max_depth")
	parser.add_argument("--max-bins", type=int, default=None, help="Override max_bins")
...
def apply_cli_overrides(config: ExperimentConfig, args: argparse.Namespace) -> None:
	...
	if args.max_depth is not None:
		config.max_depth = int(args.max_depth)

	if args.max_bins is not None:
		config.max_bins = int(args.max_bins)
...
def configure_logger(output_dir: Path) -> logging.Logger:
	logger = logging.getLogger("hgboost_audio")
...
def log_ram(logger: logging.Logger, tag: str = "") -> None:
	"""Log available system RAM from /proc/meminfo (Linux only, no deps)."""
	try:
		info: Dict[str, int] = {}
		...
		logger.info("[RAM] %s — used=%dMB avail=%dMB total=%dMB", tag, used_mb, avail_mb, total_mb)
	except Exception:
		pass  # Windows / permission issues — silently skip
...
def resolve_run_tag(config: ExperimentConfig, run_root: Path) -> str:
	...
	return datetime.now().strftime("hgboost_audio_%Y%m%d_%H%M%S")
...
def df_to_feature_matrix(
	...
	seed_base: int,
	ir_files: List[Path],
) -> Tuple["np.memmap[np.float32]", np.ndarray, Optional[Path]]:
	"""Stream features to a disk-backed memmap. Returns (memmap, labels, mmap_path).
	Caller is responsible for `del mmap` and `mmap_path.unlink()` when done.
	"""
	n = len(df_part)
	if n == 0:
		return np.empty((0, 1), dtype=np.float32), np.empty(0, dtype=np.int64), None

	# ---- probe feature dimension from first sample ----
	first_row = next(df_part.itertuples(index=False))
	first_feat = extract_feature_vector(
		wav_path=str(getattr(first_row, path_col)),
		config=config,
	...
	# ---- allocate disk-backed memmap — never fully loaded into RAM ----
	mmap_path = cache_dir / f"_mmap_{log_prefix.replace(' ', '_')}_{seed_base}.bin"
	X_mmap = np.memmap(mmap_path, dtype=np.float32, mode="w+", shape=(n, feat_dim))
	X_mmap[0] = first_feat

	y_list: List[int] = [int(getattr(first_row, label_col))]
	logger.info("%s feature %d/%d", log_prefix, 1, n)

	for i, row in enumerate(df_part.itertuples(index=False), start=1):
		if i == 1:  # already written above
			continue
	...
	X_mmap.flush()
	return X_mmap, np.asarray(y_list, dtype=np.int64), mmap_path


def _unlink_mmap(mmap: Optional["np.memmap"], path: Optional[Path]) -> None:
	"""Safely close and delete a temporary memmap file."""
	if mmap is not None:
		del mmap
	if path is not None:
		try:
...
def transform_memmap_chunked(
	X_raw: "np.memmap[np.float32]",
	scaler: "StandardScaler",
	out_path: Path,
	chunk_size: int = 8192,
) -> "np.memmap[np.float32]":
	"""Apply a fitted StandardScaler to a memmap in chunks.
	Writes result to a new memmap on disk. Peak RAM = chunk_size rows.
	"""
	n, d = X_raw.shape
	X_out = np.memmap(out_path, dtype=np.float32, mode="w+", shape=(n, d))
	for start in range(0, n, chunk_size):
		end = min(start + chunk_size, n)
		# .copy() pulls one chunk into RAM, scaler returns same shape, we write back
		X_out[start:end] = scaler.transform(np.array(X_raw[start:end], dtype=np.float32)).astype(np.float32)
	X_out.flush()
	return X_out


def build_model(config: ExperimentConfig) -> HistGradientBoostingClassifier:
...
def main() -> None:
	...
	for fold_info in cv_folds:
		...
		# --- extract features → disk-backed memmaps (never fully in RAM) ---
		x_tr_mmap, y_tr, tr_mmap_path = df_to_feature_matrix(
			fold_info["df_train"],
		...
		x_va_mmap, y_va, va_mmap_path = df_to_feature_matrix(
			fold_info["df_val"],
			path_col="resolved_wav_path",
			label_col=config.coda_label_col,
			cache_dir=cache_dir,
		...
		x_te_mmap, y_te, te_mmap_path = df_to_feature_matrix(
			fold_info["df_test"],
			path_col="file_path",
			label_col="label",
			cache_dir=cache_dir,
		...
		# --- fit scaler by reading memmap sequentially (no full copy) ---
		log_ram(logger, f"before scaler.fit fold={fold}")
		scaler = StandardScaler()
		scaler.fit(x_tr_mmap)

		# --- transform each split chunk-by-chunk into a new memmap ---
		log_ram(logger, f"before transform_tr fold={fold}")
		x_tr = transform_memmap_chunked(x_tr_mmap, scaler, cache_dir / f"_scaled_tr_{fold}.bin")
		_unlink_mmap(x_tr_mmap, tr_mmap_path); x_tr_mmap = None

		log_ram(logger, f"before transform_va fold={fold}")
		x_va = transform_memmap_chunked(x_va_mmap, scaler, cache_dir / f"_scaled_va_{fold}.bin")
		_unlink_mmap(x_va_mmap, va_mmap_path); x_va_mmap = None

		log_ram(logger, f"before transform_te fold={fold}")
		x_te = transform_memmap_chunked(x_te_mmap, scaler, cache_dir / f"_scaled_te_{fold}.bin")
		_unlink_mmap(x_te_mmap, te_mmap_path); x_te_mmap = None

		# --- train HGB directly on scaled array ---
		log_ram(logger, f"before model.fit fold={fold}")
		write_status(output_dir, run_tag, state="running", fold=fold, message="training model")
		model = build_model(config)
		
		# THE FIX: Load the ~820MB memmap entirely into RAM to prevent the Joblib multiprocessing explosion
		x_tr_ram = np.array(x_tr, dtype=np.float32)
		model.fit(x_tr_ram, y_tr)
		del x_tr_ram  # Free RAM immediately after fitting
		
		log_ram(logger, f"after model.fit fold={fold}")

		# Apply the same RAM conversion for the validation set
		x_va_ram = np.array(x_va, dtype=np.float32)
		y_score_val = model.predict_proba(x_va_ram)[:, 1].astype(np.float32)
		del x_va_ram
		...
		# Apply the same RAM conversion for the test set
		x_te_ram = np.array(x_te, dtype=np.float32)
		y_score = model.predict_proba(x_te_ram)[:, 1].astype(np.float32)
		del x_te_ram
		...
		# --- clean up scaled memmaps from disk ---
		_unlink_mmap(x_tr, cache_dir / f"_scaled_tr_{fold}.bin")
		_unlink_mmap(x_va, cache_dir / f"_scaled_va_{fold}.bin")
		_unlink_mmap(x_te, cache_dir / f"_scaled_te_{fold}.bin")
	...
	save_roc_all_folds_plot(output_dir, all_fold_results, "hgboost", logger)
